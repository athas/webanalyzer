...
\section{Målgruppe}
\label{målgruppe}
Der er adskillige mulige målgrupper og en løsning på problemet kunne
være orienteret imod --- fra hjemmebrugere der skriver personlige
sider, over de individuelle skribenter på et større website til selve
...
\section{Dybdte håndtering}

\begin{figure}
  \includegraphics[width=\textwidth]{depthtreeill.pdf}
  \caption{Ekseml på dybden af links på et simpelt websted}
  \label{depthtree}
\end{figure}

Når sider bliver analyseret og links fundet, gemmes disse med den pågældende
dybde de ligger i. Når der herefter skal trækkes ud fra denne liste bruges
bredte først
søgning\footnote{\url{http://en.wikipedia.org/wiki/Breadth-first_search}}. Da
hvis man havde brugt dybdte
førstsøgning\footnote{\url{http://en.wikipedia.org/wiki/Depth-first_search}}
på figur \ref{depthtree} og med maks crawl dybdte på 2, ville man ende med at
startside, underside X, underside Y blev analyseret, da hovedsiden også linker
direkte til underside Y bør underside Z også komme med i analysen da den ved
denne vej har dybte 2, men dette sker ikke da underside Y allerede er
analyseret og bliver ignoret ved alle fremtidig referencer pånær den første,
grundet caschen \fixme{ref til forklarende tekst omkring caschen}.
På baggrundt af dette bruges bredte først søgning hvorved dette problem
ikke opstår.
