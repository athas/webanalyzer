...
\fixme{tjek alle wikipedia links om de kan erstattes og ellers put datoer på}
\fixme{website eller websted?}
...
\fixme{Note om FRE, da det ikke er omtalt før}
...
ændringer over det meste af programmet. \fixme{Tekst der følger her
  giver ikke meget mening} Den uafhængige plads som
...
\begin{figure}
  ...
  $\blacksquare$a href=``foo''>\\
  \hspace*{5pt}$\uparrow$LexingTag\\
  <$\blacksquare$ href=``foo''>\\
  \hspace*{10pt}$\uparrow$LexingTagName\\
  <a$\blacksquare$href=``foo''>\\
  \hspace*{15pt}$\uparrow$LexingAttributes\\
  <a $\blacksquare$ref=``foo''>\\
  \hspace*{20pt}$\uparrow$LexingAttributeName\\
  <a h$\blacksquare$ef=``foo''>\\
  \hspace*{25pt}$\uparrow$LexingAttributeName\\
  <a hr$\blacksquare$f=``foo''>\\
  \hspace*{30pt}$\uparrow$LexingAttributeName\\
  <a hre$\blacksquare$=``foo''>\\
  \hspace*{40pt}$\uparrow$LexingAttributeName\\
  <a href$\blacksquare$``foo''>\\
  \hspace*{45pt}$\uparrow$LexingAttributeValueStart\\
  <a href=$\blacksquare$oo''>\\
  \hspace*{50pt}$\uparrow$LexingAttributeValue\\
  <a href=``f$\blacksquare$o''>\\
  \hspace*{55pt}$\uparrow$LexingAttributeValue\\
  <a href=``fo$\blacksquare$''>\\
  \hspace*{60pt}$\uparrow$LexingAttributeValue\\
  <a href=``foo''$\blacksquare$\\
  \hspace*{65pt}$\uparrow$LexingAttributes\\
  <a href=``foo''>$\blacksquare$\\
  \hspace*{70pt}$\uparrow$Done\\
  \caption{Eksempel genereret data af lexer}
...
og/eller \texttt{table}--tags som bliver brugt til at opsætte
hjemmesiden som ønsket. I almindelige dokumenter er vi dog ikke
interesserede i denne slags indlejring. Tekstudtrækkerens job består
således også af at fladgøre det parsetræ som den får leveret. I vores
implementering af fladgøringen gennemgår vi træet med ``inorder tree
walk'', på vej ned igennem træet opsamler vi de semantiske
informationer (f.eks. om sprog) som er angivet, sådan at vi kan
klistre dem på det tekst vi møder. Når vi møder et tekstelement
indsætter vi teksten sammen med de gældende opsamlede informationer i
den resulterende liste. Når vi møder et tag der kan adskille teksten i
afsnit, så indsætter vi et symboler der indikerer \textit{``nyt
  afsnit''} før og efter det rekursivt udtrukne indhold af det
pågældende tag. Efter fladgøringen har vi så en liste hvor teksten har
fået klistret semantisk informationer på og hvor der er et symbol hver
gang der starter et nyt afsnit. Som omtalt ovenfor bliver tabeller og
\texttt{div}--tags ofte brugt til at lave layout med, mange steder i
forbindelse med den slags tags vil der derfor blive indsat en stor
dosis \textit{``nyt afsnit''} symboler, dem fjerner vi ved at erstatte
flere på hinanden følgende symboler med et enkelt.
...
forkortelses--genkendelse. Derefter opdeler vi listen af ord i
...
\begin{longtable}[c]{p{100pt}|p{100pt}|p{100pt}|p{100pt}|p{100pt}}
\textbf{Nr.} &
\textbf{Jesper} &
\textbf{Martin} &
\textbf{Troels} \\ \hline
...
Main.sml/sig & $\surd$ & $\surd$ & $\surd$ \\ \hline
...
Sentencifier.sml/sig & & $\surd$ & \\ \hline
...
\fixme{Uopfyldte krav}
\fixme{Arbejdsfordeling}
